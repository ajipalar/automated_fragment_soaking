...
class CircleMover:
    def __init__(self, line, circle, circle_center, inner_circle, textbox):
        #print('CircleMover')
        self.line = line
    ...
    def __call__(self, event):
        ...
        self.circle.instance.update_displacement(event)
...
class Well_well_well():
	def __init__(self, wellcenterx,wellcentery,wellcenterradii, dropcenterx, dropcentery, dropradii, targetcenterx, targetcentery, targetradii, name,impath):
...
class Plate():
	...
	for well in well_names:
		plate_dict[well]=Well_well_well(600,600,400,600,600,20,0,0,0,well,'test')
...
class Simple_dog: #plotting class
	...
	def __init__(self, well_obj):
		...
		self.x_slider_axes = plt.axes([0.05, 0.1, 0.6, 0.03])
		...
		self.ax2= plt.axes([0.65,0.6,0.3,0.3])
		...
		self.target_circle = self.ax1.add_artist(MyCircle((self.wxpos,self.wypos),self,self.dr, color='b',fill=False))
		...
		self.ax3= plt.axes([0.8,0.8,0.15,0.12])
		#self.ax3.axis('off')
		...
		self.xSlider = Slider(self.x_slider_axes, 'wxpos', 0, 200, valinit=0)
		...
		self.xCID = self.xSlider.on_changed(self.f)
	...
	def update_active_circle(self,event):
		...
		plt.draw()
	def update_displacement(self,event):
		...
		sx = np.round(dx[0], decimals = 0)
		sy = np.round(dy[0],decimals = 0)

		dropx = self.drop_circle._center[0]
		dropy = self.drop_circle._center[1]
		...
		string = 'disp from well center X: %s um\ndisp from well center Y: %s um\ninner well diameter %s px\npixel size %s um' %(sx, sy,well_inner,np.round(microns_per_px,decimals=2))
		print(string)
		self.text.set_text(string)
		return sx, sy
